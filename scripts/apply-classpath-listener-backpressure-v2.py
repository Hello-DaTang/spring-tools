...
def main() -> None:
    ...
    if package.get("rootFixBuild", {}).get("patch") == "classpath-listener-backpressure-v2":
        print("Root fix already applied.")
        return

    cache = cache_file.read_text(encoding="utf-8")
    cache = replace_once(cache, "import java.time.Duration;\n", "", "remove timeout import")
    cache = replace_once(
        cache,
        "\tprivate static final Duration INITIALIZE_TIMEOUT = Duration.ofSeconds(10);\n",
        "",
        "remove registration timeout",
    )
    cache = replace_once(
        cache,
        "\tprivate Mono<Disposable> classpathListenerRequest;\n",
        "\tprivate Mono<Disposable> classpathListenerRequest;\n\tprivate Disposable classpathListenerRegistration;\n",
        "add registration subscription handle",
    )
...
\tpublic Collection<? extends IJavaProject> all()"""
    ...
    if count != 1:
        raise RuntimeError("Unable to replace classpath listener state machine")
    ...
    manager = replace_once(
        ...
        "clean up genuine failure or explicit cancellation",
    )
    ...
    package["repository"] = {
        "type": "git",
        "url": "https://github.com/Hello-DaTang/spring-tools.git",
    }
    package["rootFixBuild"] = {
        "upstream": "https://github.com/spring-projects/spring-tools",
        "fork": "https://github.com/Hello-DaTang/spring-tools",
        "branch": "rootfix/classpath-listener-backpressure-v2",
        "patch": "classpath-listener-backpressure-v2",
    }
    package_file.write_text(json.dumps(package, indent=2) + "\n", encoding="utf-8")

    print("Applied classpath-listener backpressure root fix v2.")
